...
export class TitleScene extends Phaser.Scene {
  ...
  create() {
    ...
    if (this.textures.exists('title-bg')) {
      const src = this.textures.get('title-bg').source[0]
      this.bgTexW = src.width
      this.bgTexH = src.height
      this.bg = this.add.image(cx, H / 2, 'title-bg').setDepth(0)
      this.bg.setScale(Math.max(W / this.bgTexW, H / this.bgTexH))
    } else {
