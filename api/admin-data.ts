...
export default async function handler(req: any, res: any) {
  ...
  try {
    // 全上書き行（下書き含む）を返す
    if (action === 'list') {
      const { data, error } = await db.from('ebt_data_overrides').select('*')
    ...
    if (action === 'save') {
      ...
      const { error } = await db.from('ebt_data_overrides').upsert({
        category, ref,
    ...
    if (action === 'unpublish') {
      const { category, ref } = req.body ?? {}
      const { error } = await db.from('ebt_data_overrides')
