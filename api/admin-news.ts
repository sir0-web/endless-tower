...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    if (action === 'create') {
      ...
      const { data, error } = await db.from('ebt_announcements').insert({
    ...
    if (action === 'update') {
      ...
      const { data, error } = await db.from('ebt_announcements')
    ...
    if (action === 'delete') {
      const { id } = req.body ?? {}
      if (!Number.isFinite(Number(id))) return res.status(400).json({ error: 'id 必須' })
      const { error } = await db.from('ebt_announcements').delete().eq('id', Number(id))
      if (error) return res.status(500).json({ error: error.message })
